...
TEST(Ds18Poll, NotReadyImmediatelyAfterRequest) {
  ...
  EXPECT_FALSE(ds18_poll_conversion_ready(state, 1749));
}

TEST(Ds18Poll, ReadyAfterConversionWindow) {
  Ds18PollState state{};
  ds18_poll_begin_request(state, 1000);
  EXPECT_TRUE(ds18_poll_conversion_ready(state, 1750));
}
...
TEST(Ds18Poll, ReRequestResetsWindow) {
  ...
  EXPECT_FALSE(ds18_poll_conversion_ready(state, 2500));
  EXPECT_TRUE(ds18_poll_conversion_ready(state, 2750));
}
